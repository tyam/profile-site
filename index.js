...
const Preface = ({step, actions}) => (
  <div key="preface" 
    ...
    <Block no={2} step={step}>
      ...
      年間決済高ウン十億円・100万ユーザーのシステムの責任者を6年経験。大規模システムの運用・管理の知識があります。
    </Block>
...
const paper2 = ({state, actions}) => (
  <Paper p={2} title="略歴" state={state} actions={actions}>
    ...
    そして、販促を行い人をたくさん集めることも出来ませんでした。そもそも、販促に大金を費やす必要があることすら知りませんでした。ユーザーがいないことは、オンラインゲームの世界では致命的なことです。<br />
    私は世間知らずでした。</p>
    </div>
    </div>
    <div className="Grid">
    ...
    <div className="text">
    <p>資金が尽きて、次の身の振り方を考えているちょうどその時に、以前のクライアントから「うちに来ないか」と誘われ、入社しました。</p>

    <p>その会社はマルチテナント型のECサイトを運営している小さな会社です。小さいながらも年間決済高はウン十億円にも上りました。<br />
    入社2年目からはシステム部門の責任者になり、担当する仕事も手を動かす仕事から人を動かす仕事へ比重が移りました。<br />
    全社的なサーバー移転や4年がかりのシステムリニューアルなど、様々なプロジェクトを完遂してきました。</p>

    <p>ただ、勤め始めて6年経った頃から、私の中に不満と不安が芽生えていました。<br />
    そこは小さな事業会社です。プロジェクトも少なく、仕事の中で技術を伸ばすのは困難です。小さな会社の中で、裁量も固定化されていました。<br />
    「この会社ではもうやり甲斐を感じらないのではないか」<br />
    そんな思いから、私は退社を決めました。</p>

    <p>そうして、2018年4月から、私はフリーエンジニアとして活動しています。</p>

    <p>2018年10月現在、私は42歳です。<br />
    プライベートでは結婚して子供もいます。<br />
...
const paper3 = ({state, actions}) => (
  <Paper p={3} title="スキル・興味" state={state} actions={actions}>
    <Slider paper={3} state={state} actions={actions}>
      ...
      <div class="body">
      <p>サーバーは専らLinuxです。<br />
      プログラミング言語はPHPとJavaScript（TypeScript）がメインです。<br />
      フレームワークはradarphp、hyperapp、Laravel、React.jsをよく使います。<br />
      クラウドはGCPとAWSの経験があります。</p>
      <p>設計においてはレイヤー分割・モジュール分割を適切に行い、柔軟かつ持続可能なシステムの構築を目指します。</p>
      </div>
      </div>
      </div>
      <div>
      ...
      サービスを構築する際には、そういった技術の利用を最初に考えます。</p>
      </div>
      </div>
      </div>
    </Slider>
...
const paper4 = ({state, actions}) => (
  <Paper p={4} title="プロフィール" state={state} actions={actions}>
    ...
    <dd class="desc">42歳（2018年10月現在）</dd>
    <dt class="title">住まい</dt>
    <dd class="desc">千葉県千葉市</dd>
    <dt class="title">職業</dt>
    <dd class="desc">システムエンジニア／プログラマー<br /><span class="Note">フリーのエンジニアとして活動中</span></dd>
